<?php
namespace SayItWithAGift\Options\Observer;
...
class OptionSaveAfter implements ObserverInterface {
	...
	function execute(\Magento\Framework\Event\Observer $observer) {
		...
		if ($object->getResourceName() == 'Magento\Catalog\Model\ResourceModel\Product\Option\Value') {
			...
			else if ($object->getImageSavedAs() && $object->getDeleteImage() != 1) {
				$image = $object->getImageSavedAs();
			}
			if (!empty($image) || $object->getDeleteImage() == 1) {
				$model = $this->_objectManager->create(sV::class)->load(
					$object->getId(), 'option_type_id'
				);
	...
	protected function _moveImageFromTmp($file) {
		...
		$this->_mediaDirectory->renameFile(
			$this->_mediaConfig->getTmpMediaPath($file), $this->_mediaConfig->getMediaPath($destFile)
		);
